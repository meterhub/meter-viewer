...
使用方式:
    uv run python tools/validate_labels_yaml.py --yaml data/labels.yaml --root data
"""
...
def validate_labels_yaml(yaml_path: Path, root_path: Path):
    """验证 YAML 标签文件的完整性"""
    if not yaml_path.exists():
        print(f"错误: YAML 文件 {yaml_path} 不存在")
        return False
    ...
    if all_samples_valid:
        print("✅ 验证通过! YAML 标签文件格式正确")
        return True
    else:
        print("❌ 验证失败! 请检查 YAML 文件")
        return False


def check_file_integrity(yaml_path: Path):
    """检查文件完整性"""
    try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='验证 YAML 标签文件')
    parser.add_argument('--yaml', default='data/labels.yaml', help='YAML 标签文件路径 (默认: data/labels.yaml)')
    ...
    # 检查文件完整性
    if not check_file_integrity(yaml_path):
        sys.exit(1)
    
    # 验证标签内容
    success = validate_labels_yaml(yaml_path, root_path)
