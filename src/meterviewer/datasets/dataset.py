...
def create_dataset_func(check_imgs: t.Callable[[T.ImgList], None]):
    def inner(
        ...
        target_size: t.Tuple[int, int] = (192, 32),  # 目标尺寸
    ):
        imgs = []
        labels = []
...
def generate_block_img(
    ...
    target_size: t.Tuple[int, int] = (32, 192),  # 目标尺寸 (高度, 宽度)
    left_replace_width: int = 96,  # 替换区域的总宽度
    ...
    new_label = "".join([digit for digit, _ in replacements] + list(img_a_label[3:]))
